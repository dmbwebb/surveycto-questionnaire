...
| Function | Signature | Return type | Description | Example | Docs |
| --- | --- | --- | --- | --- | --- |
| `duration()` | `duration()` | number | Seconds spent filling or editing the current submission. In `calculate_here`, `once(duration())` captures time first reaching that point. This is the ONLY reliable way to record interview length: a plain `calculate` subtracting `${start}` from `${end}` exports empty, because `end` is stamped at finalisation and plain calculates are not re-evaluated then. | `once(duration())` | [Working with dates and times](https://docs.surveycto.com/02-designing-forms/01-core-concepts/09.expressions.html#working-with-dates-and-times) |
