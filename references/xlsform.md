...
| `audio audit` | Hidden | SurveyCTO-specific invisible audio recording of survey administration. | `appearance` parameters include `p=#`, `s=#`, `s=#-#`, `s=fieldname`, `d=#` (seconds), and `d=fieldname` (stop when swiping away from that field); separate parameters with semicolons. Anchors must be visible fields outside any repeat, named bare rather than as `${field}`. An audit does NOT resume after Edit Saved Form, so one whole-survey audit loses every parked-and-resumed interview: segment long forms, see [Audio audits: segment long recordings](form-patterns.md#audio-audits-segment-long-recordings). Collect only, not web. | [audio audit](https://docs.surveycto.com/02-designing-forms/01-core-concepts/03ze.field-types-audio-audit.html), [missing recordings](https://support.surveycto.com/hc/en-us/articles/360057544213-Why-are-my-audio-audit-recordings-missing) |
...
| `p=#;s=#;d=#`, `p=#;s=#-#;d=#`, `s=startfield;d=endfield` | `audio audit` | Samples and bounds invisible audio audits by time or field. `d=` is seconds when numeric and a stop field when it names a field. Drop `p=` on field-anchored rows: only time-based (`p=`) audits are mutually exclusive, and question-based ones all fire. Segment long forms into one audit per section, since an audit does not resume after Edit Saved Form: [Audio audits: segment long recordings](form-patterns.md#audio-audits-segment-long-recordings). | [audio audit](https://docs.surveycto.com/02-designing-forms/01-core-concepts/03ze.field-types-audio-audit.html) |
