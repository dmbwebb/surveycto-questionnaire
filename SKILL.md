...
- **Malformed expression blockers.** The checker flags active expressions with duplicated boolean operators (`or  or`) or malformed quoted choice codes inside `selected()` (for example `selected(${x}, '-997)`).
