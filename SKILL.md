...
- **Media-only fixes still need a greater form version.** SurveyCTO rejects replacement uploads when `settings.version` is not lexically greater than the deployed version, even if the XLSForm logic is unchanged and only `--media` changed. Before live media redeploys, export/read the `settings` tab and compare against `/forms/{form_id}/files`; bump a static version cell or ensure the formula export resolves above the deployed version. A timestamp bumper that only compares against the version *in the file* can also re-emit a version the server already has — same minute, same string, same refusal.
- **The upload endpoint caps total payload size.** Measured on kilongajfl 2026-09-09: 2.2 MB accepted, 8.5 MB refused with `Maximum upload size exceeded`. **Attached media accumulates across uploads and survives a version replacement**, so a form whose full narration set exceeds the cap is still deployable — upload the definition with only the media that is new or whose content changed, and the rest stays attached. Verify with `/forms/{form_id}/files`, which lists each attachment name twice.
...
- **`regex()` is a whole-string match** (JavaRosa `Pattern.matches`, same in the web test view): `regex(${x}, ' wife_')` is always false; use `regex(${x}, '.* wife_.*')` or `^...$` anchors. Handy for select_multiple prefix tests: `regex(concat(' ', ${sel}, ' '), '.* wife_.*')`.
- **No `${field}` references in constraint messages.** They compile to `<output>` in the itext, but the web test view shows a raw `${N}` placeholder in the red message (SQB recruiter form, 2026-08-18); labels/hints substitute fine. Word constraint messages without references.
...
- **Moving a contiguous block of rows: use `batchUpdate` with one `moveDimension` request.** `gsheet_edit.py` has no built-in "move rows" primitive, but the Sheets API does it atomically via `moveDimension`. Use this for module reorderings (e.g. shoving a contamination-prone module to the end of a 1500-row form) — far safer than read+insert_row_at-loop+delete_row, which is many API calls and risks partial state. Source range is 0-indexed half-open. `destinationIndex` is interpreted in the **original** (pre-move) coordinate space — it's the index *before which* the moved rows will appear, so to land moved rows just before original 1-indexed row `R`, pass `destinationIndex = R - 1`. Pattern:
  ...
  ```
  After the move, total row count is unchanged and cells in the moved range follow their content (so any references to fields inside the moved block remain valid). `${...}` references work by name, not by row position, so XLSForm logic is unaffected by the reorder. ⚠️ **On a deployed form with data, only reorder within the same group nesting.** SurveyCTO tracks each field by its full path including enclosing groups — moving fields INTO or OUT OF a group/repeat, renaming a group, or changing repeat enclosure between versions breaks the linkage to already-collected data (exports treat it as a different field). Concretely, the wide CSV export then returns BLANK for every pre-move submission of those fields while the v2 wide JSON export still carries them (surveycto-data-analysis SKILL.md §Raw export gotchas has the backfill recipe) — so after any redeploy, diff non-missing counts per column between the old and new export before trusting the CSV. Whole-module moves that keep every field inside its original group are safe.
...
- **`--auth auto` silently falls back to Chrome when `$SURVEYCTO_USERNAME` is unset** and then fails with "your SurveyCTO chrome authentication is not valid" even though a Keychain item exists (the item is stored per server+username, so auto mode cannot find it without the account). On Duncan's Macs the account is `duncan.webb@psemail.eu` (exported in the shared zshrc since 2026-08-15); in a bare environment pass `--username duncan.webb@psemail.eu --auth keychain`. The same applies to `load_session(server, username=..., auth_mode='keychain')` in ad-hoc scripts — without it `/forms/{id}/files` returns a 401 HTML page.
- **Server-compile a build without touching devices: upload it as a draft** (`--update <form_id> --draft`). The live version and the served XML stay unchanged; the compiled draft is at `GET /forms/d$<form_id>/xml` (draft form id = `d$` + form id; `?draft=true` on the normal URL is ignored) and `/forms/<form_id>/files` reports it under `xForm.draftVersion` and `draftGroupFiles.definitionFile`. Redeploying the same file for real afterwards (same version number as the draft) is accepted and clears the draft slot. Used 2026-08-15 as the substitute for a browser test when the Chrome extension was unreachable. A brand-new form can also start life as a draft (`--draft` with no `--update`, media attached); a later plain `--update <id>` upload deploys it, carries the draft's attachments over, and after that `/forms/d$<id>/files` returns JSON without an `xForm` key (2026-08-30, AI Health main baseline).
- **Read deployment status from `xForm.formStatus`, not `xForm.status`.** In `GET /forms/{form_id}/files`, verify the deployed `xForm.version`, `xForm.formStatus` (normally `READY`), `xForm.deployed`, and whether `xForm.draftVersion` remains populated. The object has no reliable generic `status` field.
- **Do not route activity solely from a numeric count that permits a negative unknown sentinel** such as `-97`. Conditions like `${count} = 0` and `${count} > 0` both skip the unknown case. When downstream logic needs to know whether any activity occurred, ask or derive a separate yes/no flag for sentinel responses and route on the combined activity indicator.
- **Recover the exact xlsx of the DEPLOYED (or any earlier) version from the server**: `GET /forms/{form_id}/files` (console path at the server root, NOT under `/api/v1/`, which 404s) → `deployedGroupFiles.definitionFile.downloadLink` (and `previousDefinitionFiles[]` for older versions, each with `formVersion` + `downloadLink`); fetch with the same authenticated session (`load_session(server, username=...)`). **Every entry also carries `actor` (the uploader's login) and `dateStr`**, so this one call answers "who deployed this and when" for the whole version history — use it instead of the console's Previous-deployed-versions page when a form's live version has moved since your last known deploy and you need to identify a collaborator's hand edit before rebuilding (2026-09-09). Needed when the user's edit list quotes question IDs from what the tablets show and the local build has since been renumbered or moved on (2026-08-17: local was an undeployed round ahead of live). Convert with `surveycto_to_txt.py` and map IDs → field names from that copy, never from the local build.
- **Choice labels may reference `${field}`** (e.g. `Only ${db_religion_group}`); SurveyCTO compiles them to `<output value=...>` inside the itext, so option text can be personalised from an earlier answer or calculate. Verified in the compiled XForm 2026-08-15.
- **A visible field's `calculation` (prefill/dynamic default, e.g. an editable ID prefilled `concat(${enumerator_id}, '-', format-date(today(), '%Y%m%d'), '-')`) compiles as a `calculate` attribute on the `<input>` BODY element, not on the `<bind>`** — a bind-only readback wrongly concludes the prefill was dropped. Check `<input ref=... calculate=...>` when verifying prefills in compiled XForm (2026-08-20, SQB recruiter Round 15).
- **When walking group nesting in a survey sheet, match types exactly** (`begin group`/`begin repeat`/`end group`/`end repeat`) — `type.startswith('end')` also matches the metadata `end` timestamp field and corrupts the stack.
...
- `xForm.formStatus` — The form's current server status (normally `READY` after a successful deployment). Do not test `xForm.status`; use `formStatus` together with `xForm.deployed` and `xForm.draftVersion`.
...
When form B is a heavy edit of form A (e.g. a pilot variant: ~150 deletions, moved blocks, new modules), do NOT mutate a copy incrementally. Write ONE re-runnable Python build script that `shutil.copy`s the source form fresh and applies every transformation (delete-by-name bottom-up, capture+delete+insert to move blocks, insert-after-named-anchor for new rows, choice-list edits, then auto-prune unused choice lists by scanning `select_one/select_multiple` types). Each fix iteration = edit script → re-run → re-run checker. Benefits: deterministic rebuilds, reviewable diff of intent, audit fixes append as a phase-2 section, and the source form stays pristine. Worked example: `AI HEALTH/piloting/transport_pilot/questionnaires/build_transport_baseline.py` (Jan baseline → transport baseline; group split, scale-block relocation, ~90 new rows, settings/version/instance_name). To restore blocks from a THIRD form (not the copy source), port rows across workbooks by name instead of retyping labels: copy the first ~22 columns by index after asserting both header layouts match, and copy the label/label:Hindi cell FONTS too — verified Hindi stays black while your insert helper reddens only genuinely new text (worked example: `port_jan_rows`/`apply_jan_fonts`/`port_jan_choices` in `AI HEALTH/piloting/planning_info_test/questionnaires/build_infoplan_baseline.py`). Three more rules from the AI Health main-study build (2026-08-30): apply cross-cutting gates (study arm, village-level factor) as the LAST step of a section — any later `set_cell(name, C_REL, ...)` silently overwrites an earlier `and_rel`, which leaked information-arm items into the control arm until a walkthrough caught it; assert (don't eyeball) that question-ID prefixes are unique AND that each ID labels the same row in every language (`CM.1` was used by both the medicines and the transport-companion blocks) — they confuse enumerators and break label-keyed tooling. Make it a hard assertion because the usual way IDs collide is a blanket `str.replace("Q6.7:", "Q6.9:")` renumber that also sweeps up a row you inserted moments earlier; renumber highest-ID-first, and expect the assertion to catch both a collision and a row that picked up an ID in one language only (it caught both, twice, in one SQB session). Walk every arm-by-village path headlessly (surveycto-tester `surveycto_walk.py`) before calling a build done. Two gotchas: never hand-edit the generated xlsx (rebuild wipes it — put a note in the project CLAUDE.md); write a computed static `settings.version` string from `datetime.now()` in the script rather than the NOW() formula (openpyxl can't cache formula values, so a rebuild would otherwise upload a literal `=TEXT(...)` string).

**Taking over a collaborator's form that exists only on the server:** download the deployed definition (`/forms/{id}/files` → `deployedGroupFiles.definitionFile.downloadLink`, which also names the `actor` and `dateStr`), then make Phase 0 of the build script *reproduce that baseline and assert it cell-for-cell* against the served file before any edit phase runs — extract the baseline rows into a generated literal module so the reproduction is specified in code rather than implied by a `shutil.copy`. Give it a `--baseline-only` flag. This converts "I think my edits are surgical" into a checkable claim, makes every later phase a reviewable delta, and catches the case where you were editing a stale local copy. Still copy the served workbook as the write target so SurveyCTO's help sheets, conditional formatting and column widths survive.
...
2. **Unsupported functions:** SurveyCTO doesn't support `starts-with()`, `contains()`, `substring-before()`, `substring-after()`, `upper-case()`/`lower-case()`, or `translate()` — even though [ODK docs](https://docs.getodk.org/form-operators-functions/) list several of them, SurveyCTO's JavaRosa parser rejects them with "cannot handle function 'substring-after'". Use `substr(string, 0, N) = 'prefix'` and `regex(string, '.*pattern.*')` for matching, and `selected-at(string, N)` (zero-indexed, space-separated) for extraction. **When in doubt, check [SurveyCTO's expressions reference](https://docs.surveycto.com/02-designing-forms/01-core-concepts/09.expressions.html), NOT the ODK docs** — SurveyCTO's XPath function set is a strict subset of ODK's. With neither `upper-case()` nor `translate()`, a form **cannot normalise typed text at all** — no case folding, no stripping stray spaces. Handle case in the data instead (emit upper- and lower-case key rows in the `pulldata` CSV so either spelling resolves) and block stray characters with a constraint such as `regex(., '^[A-Za-z0-9-]+$')` rather than trying to clean them in an expression. The server rejects each unsupported function one at a time, so a form using several costs one failed upload per function: grep the built workbook for them before uploading.

3. **A conditional row never fires if its trigger is on the same `field-list` screen.** A note (or any row) whose `relevance` depends on a field in the same `field-list` group stays hidden even after that field is answered: the group renders once, and nothing re-renders before the enumerator leaves it. Verified on a live draft — a re-explanation gated on a teach-back coded "incorrect" and a warning gated on two eligibility answers both stayed invisible with their triggers set. It is silent, so a walkthrough that only checks "the form advances" will not catch it. Put the conditional row **outside** the `field-list`, on its own screen after it, and remember the ordering consequence: anything the respondent must hear *before* answering must also come before the question it informs, which usually means splitting one long screen into read → correct → decide. Watch too for the blank-value trap on such a row: an unanswered field is `''`, and `'' != 1` is true, so gate with `string-length(${f}) > 0 and ${f} != 1`.

4. **Integers cap at nine digits.** Longer values (phone numbers, long IDs) silently break — use `text` with `appearance: numbers` / `numbers_phone` instead, which shows the numeric keyboard but stores text (also preserves leading zeros).

5. **Extracting structured data from a field plug-in's output:** Use the `plug-in-metadata()` + `selected-at()` pattern. The plug-in calls `setMetaData(spaceSeparatedString)` (where any value containing spaces has its spaces replaced with `_`), and a calculate field then reads `selected-at(plug-in-metadata(${plugin_field}), N)` for the Nth value (zero-indexed). Direct JSON parsing in the form is impossible — there's no `substring-before/after`, `regex-replace`, or JSON parser.

6. **Dynamic constraint messages work:** `constraint message` (and its language columns) may contain `${field}` references; the server compiles them to `<output value=...>` in the itext (verified 2026-08-17). `substr(string, start, end)` is 0-based, start inclusive, end exclusive: on `KMP1-OF-07A`, `substr(., 5, 7)` = `OF` and `substr(., 8, 10)` = `07`.
...
- **A permanently irrelevant `calculate` does not initialize its value.** Do not retire a field by setting `relevance = 0` while leaving its calculation as the switch for current downstream fields: the value stays blank and every dependent branch can disappear even when the checker reports zero errors. Preserve the retired field for export continuity, add a separate always-relevant calculate for the current switch, rewire downstream references, and walk the critical path in SurveyCTO's real test view before deployment.
...
- Create a dataset: `POST /groups/{parentGroupId}/create-new-dataset` (root group = 1), **multipart** form-data with `dataset_exists=0`, `dataset_id`, `dataset_title`, `dataset_type=SERVER`, `dataset_upload_mode=append`, blank `authToken`, and `locationContext={"parentGroupId":1,"siblingAbove":null,"siblingBelow":null}`. Plain `data=` fails with "request was not multipart". The endpoint can return HTTP 200 with an application-level failure, so require JSON `code == 200`, then verify via the list endpoint.
...
- Attach/detach a dataset to a form: `POST /datasets/{datasetId}/attach/{formId}/{on|off}` (header `X-Requested-With: XMLHttpRequest`; 200 on success). For a draft-only form, use `d$<formId>`; after deployment SurveyCTO should migrate the outgoing attachment to the live form ID, but re-list datasets and verify. Far more reliable than the attach checklist. Works before the dataset has any data; a form referencing a not-yet-attached dataset still uploads fine (console flags the missing attachment rather than rejecting).
...
- **A rate-limited response is a dict, so `len(r.json())` lies.** A successful
  pull returns a JSON *array* of submissions; an error returns
  `{"error": {...}}`, whose `len()` is 1. Code that reports
  `len(r.json())` as a submission count silently turns "rate limited" into
  "one submission" — which reads as real data landing on a form that should
  still be empty. Always branch on
  `isinstance(d, dict) and "error" in d` before counting.
