...
- **Recover the exact xlsx of the DEPLOYED (or any earlier) version from the server**: `GET /forms/{form_id}/files` (console path at the server root, NOT under `/api/v1/`, which 404s) → `deployedGroupFiles.definitionFile.downloadLink` (and `previousDefinitionFiles[]` for older versions, each with `formVersion` + `downloadLink`); fetch with the same authenticated session (`load_session(server, username=...)`). Needed when the user's edit list quotes question IDs from what the tablets show and the local build has since been renumbered or moved on (2026-08-17: local was an undeployed round ahead of live). Convert with `surveycto_to_txt.py` and map IDs → field names from that copy, never from the local build.
